<template>
  <div class="CouponModel">
    <div class="ModelContainer fixed">
      <div class="masks" @click="handCouponHide(false)"></div>
      <div class="Coupon animated fadeInRight faster" >
        <div class="CouponList">
          <div class="list" v-for="(item,index) in coupon.List" :key="index" @click="handelUse(item)">
            <div class="list-info" >
              <h2><text>￥{{item.Price}}</text>{{item.Name}}</h2>
              <p>{{item.Remarks}}</p>
              <label>有效期至{{item.validity}}</label>
            </div>
            <div class="list-btn">
              <label>立即使用</label>
            </div>
...
export default {
  ...
  computed: {
    // ZongPrice(){
    //     return (parseInt(this.commodityModel.Model[this.ModelIndex].Price)*parseInt(this.ModelNumber))+this.CouponNumber
    // },
    commodityModel () {
      return {
  ...
  methods: {
    handelUse (item) {
      this.$emit('handelUse', item)
    },
    handNoUser () {
      this.$emit('handNoUser', 0)
    },
    handCouponHide (State) {
      this.$emit('CouponModelState', State)
    }
  }
}
</script>
    ...
    .Coupon {
      ...
      .CouponList{
        .list{
          ...
          border-radius: 20rpx;
          margin:20rpx 0rpx;
          box-shadow: 0 0 8rpx 2rpx rgba(0, 0, 0, 0.05);
          padding: 20rpx;
          display: flex;
